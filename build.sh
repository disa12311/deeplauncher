#!/usr/bin/env bash
# build.sh — robust build script for WASM
set -euo pipefail
...
ts(){ date +"%Y-%m-%d %H:%M:%S"; }
echo "[$(ts)] build.sh OUT_DIR=${OUT_DIR} PUBLIC_WASM_DIR=${PUBLIC_WASM_DIR}"
...
echo "[$(ts)] Found crate: ${CRATE_TOML} (dir: ${CRATE_DIR})"
